...
if test "$#" -lt 4; then
    echo ""
    echo "Use: ./iq_collect <modulation> <TX grid node #> <RX grid node #> <iperf/mgen>"
    echo ""
    sleep 1
...
tmux new-session \; \
    ...
    send-keys -t 2 "timeout 15 sudo python/ecet680-radio.py --auto-soft-tx-gain 20 -v -G 20 -R 2 -d -b 1e6 -n 2 -i 1 --rx-antenna TX/RX -f 1.3${gn_rx}e9 -l logs --log-iq -m $modulation --arq --lifo --verbose-packet-trace --slot-size 0.05 --guard-size 0.005" C-m '' \; \
    ...
    send-keys -t 0 "timeout 15 sudo python/ecet680-radio.py --auto-soft-tx-gain 20 -v -G 20 -R 2 -d -b 1e6 -n 2 -i 2 --rx-antenna TX/RX -f 1.3${gn_rx}e9 -m $modulation --arq --lifo --verbose-packet-trace --slot-size 0.05 --guard-size 0.005" C-m '' \; \
    send-keys -t 3 "sshpass -p 'kapilrocks' ssh -X root@$gn_rx_ip" C-m '' \; \
    send-keys -t 3 'sleep 3 && mgen port 5001' C-m \; \
